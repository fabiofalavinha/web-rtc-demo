var PeerManager = (function () {
    ...
    var localId,
        config = {
            peerConnectionConfig: {
                iceServers: [
                    { "url": "turn:ec2-54-243-184-139.compute-1.amazonaws.com" },
                    { "url": "turn:ec2-54-243-184-139.compute-1.amazonaws.com:3479" }
                ]
            },
    ...
    function addPeer(remoteId) {
        ...
        peer.pc.onicecandidate = function(event) {
            if (event.candidate) {		  
                ...
                if (candidate.indexOf("relay") < 0) { // if no relay address is found, assuming it means no TURN server
                    console.log("ICE  -  No TURN server");				
                } else {
                    console.log("ICE  - i'm using the TURN server to relay");
                }		  
        ...
        peer.pc.oniceconnectionstatechange = function(event) {
            var iceConnectionState = event.srcElement  /* chrome */ || event.target /* firefox */;
